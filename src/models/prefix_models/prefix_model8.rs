...
impl<C: Counter> PrefixModel8<C> {
    pub fn new(counter: C) -> Self {
        Self { stats: vec![counter; 1 << 8], ctx: 1 }
    }
}

impl<C: Counter> AdaptiveModel for PrefixModel8<C> {
    fn predict(&self) -> u16 {
...
impl<C: Counter> FreezeModel for PrefixModel8<C> {
    ...
    fn freeze(&self) -> Self::Frozen {
        ...
        StaticPrefixModel8 { stats, ctx: 1 }
    }
}

impl SerializableModel for StaticPrefixModel8 {
...
impl<C: Counter> CtxModel for CtxPrefixModel8<C> {
    fn predict(&self) -> u16 {
        self.stats[self.ctx].p()
    }
