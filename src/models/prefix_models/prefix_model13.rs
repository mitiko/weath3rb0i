...
impl<C: Counter> AdaptiveModel for PrefixModel13<C> {
    fn predict(&self) -> u16 {
    ...
    fn update(&mut self, bit: u8) {
        ...
        if self.ctx ^ self.nibble >= 1 << 12 {
            self.nibble ^= 1 << 12;
            self.ctx &= 0xff;
            self.ctx |= self.nibble | (1 << 8);
        }
    }
}
...
impl<C: Counter> FreezeModel for PrefixModel13<C> {
    ...
    fn freeze(&self) -> Self::Frozen {
        ...
        StaticPrefixModel13 { stats, ctx: 1 << 8, nibble: 0 }
    }
}

impl SerializableModel for StaticPrefixModel13 {
...
/// ```
#[derive(Clone)]
pub struct CtxPrefixModel13<C: Counter> {
    stats: Vec<C>,
    ctx: usize,
    lead: usize,
    nibble: usize,
...
impl<C: Counter> CtxPrefixModel13<C> {
    pub fn new(counter: C) -> Self {
        Self {
            stats: vec![counter; 1 << 13],
            ctx: 1 << 8,
...
impl<C: Counter> CtxModel for CtxPrefixModel13<C> {
    fn predict(&self) -> u16 {
        self.stats[self.ctx].p()
    }
