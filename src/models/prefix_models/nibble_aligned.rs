...
macro_rules! define {
    ($name:ident, $bits:expr, $mask:expr) => {
        ...
        impl<C: Counter> $name<C> {
            pub fn new() -> Self {
                Self { stats: vec![C::new(); 1 << ($bits + 2)], ctx: 0, alignment: 0 }
            }
        }
...
define!(NibbleAlignedPrefixModel4, 4, 0xF);
define!(NibbleAlignedPrefixModel7, 7, 0x7F);
define!(NibbleAlignedPrefixModel8, 8, 0xFF);
define!(NibbleAlignedPrefixModel12, 12, 0xFFF);
define!(NibbleAlignedPrefixModel16, 16, 0xFFFF);
