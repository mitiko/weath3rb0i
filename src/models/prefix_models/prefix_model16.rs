...
impl<C: Counter> PrefixModel16<C> {
    pub fn new(counter: C) -> Self {
        Self { stats: vec![counter; 1 << 16], ctx: 1 << 8 }
    }
}

impl<C: Counter> AdaptiveModel for PrefixModel16<C> {
    fn predict(&self) -> u16 {
...
impl<C: Counter> FreezeModel for PrefixModel16<C> {
    ...
    fn freeze(&self) -> Self::Frozen {
        ...
        StaticPrefixModel16 { stats, ctx: 1 << 8 }
    }
}

impl SerializableModel for StaticPrefixModel16 {
...
/// ```
#[derive(Clone)]
pub struct CtxPrefixModel16<C: Counter> {
    stats: Vec<C>,
    ctx: usize,
    lead: usize,
}

impl<C: Counter> CtxPrefixModel16<C> {
    pub fn new(counter: C) -> Self {
        Self {
...
impl<C: Counter> CtxModel for CtxPrefixModel16<C> {
    fn predict(&self) -> u16 {
        self.stats[self.ctx].p()
    }
