...
impl<C: Counter> AdaptiveModel for PrefixModel9<C> {
    fn predict(&self) -> u16 {
    ...
    fn update(&mut self, bit: u8) {
        ...
        if self.ctx ^ self.nibble >= 1 << 8 {
            self.nibble ^= 1 << 8;
            self.ctx &= 0xf;
            self.ctx |= (1 << 4) | self.nibble;
        }
    }
}
...
impl<C: Counter> FreezeModel for PrefixModel9<C> {
    ...
    fn freeze(&self) -> Self::Frozen {
        ...
        StaticPrefixModel9 { stats, ctx: 1 << 4, nibble: 0 }
    }
}

impl SerializableModel for StaticPrefixModel9 {
...
/// ```
#[derive(Clone)]
pub struct CtxPrefixModel9<C: Counter> {
    stats: Vec<C>,
    ctx: usize,
    lead: usize,
    nibble: usize,
...
impl<C: Counter> CtxPrefixModel9<C> {
    pub fn new(counter: C) -> Self {
        Self {
            stats: vec![counter; 1 << 9],
            ctx: 1 << 4,
...
impl<C: Counter> CtxModel for CtxPrefixModel9<C> {
    fn predict(&self) -> u16 {
        self.stats[self.ctx].p()
    }
